...
// ensure auth state is observed (so currentUser is available)
...
function saveMinutes(group, content, subcategory, minutes) {
  ...
  if (!user) {
    alert('You must be logged in to save training.');
    return;
  }

  const emailKey = emailToKey(user.email); // helper from firebase-init.js
  const path = `trainings/${emailKey}/${group}`;
  const entry = {
    content,
    subcategory,
  ...
  // push the entry into Realtime DB
  database.ref(path).push(entry)
    .then(() => {
      alert("Saved!");
    })
    .catch(err => {
